...
class PandSBot(Commander):
    ...
    def initialize(self):
        ...
        ##result = an.buildLOS()
        self.attackingPaths = self.levelAnalysis.getBestBreakingPoints([self.ourSpawn], self.game.enemyTeam.flag.position, self.level.firingDistance*0.75, self.level.firingDistance*1.5, 4) #int(self.countBot)+2*0)
        ...
        self.breakingPoints = self.levelAnalysis.getBestBreakingPoints(startPoints, self.game.team.flag.position, self.level.firingDistance*0.75, self.level.firingDistance*1.5, 4)#int(self.countBot)+2*0)
