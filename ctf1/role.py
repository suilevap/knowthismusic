...
Time_SwitchToDefend = 2.0
Time_SwitchState = 1.0
...
def Command_ChargeHomeSafe(commander, bot):
    target = commander.game.team.flagScoreLocation
    path = commander.getSafePath(bot.position, target)
    if len(path)==0:
        return False
    commander.issue(  commands.Charge, bot, path, description = 'Charging home safe (ATTACKER)')
    return True

def Command_TryChargeHomeSafe(commander, bot):
    print 'try change path'
    target = commander.game.team.flagScoreLocation
    path = commander.getSafePath(bot.position, target)
...
def Command_RunHomeSafe(commander, bot):
    ...
    if len(path)==0:
        return False
    commander.issue(  commands.Move, bot, path, description = 'Running home safe (ATTACKER)')
    return True
...
def Command_RunHome(commander, bot):
    ...
    return True
def Command_ChargeHome(commander, bot):
    target = commander.game.team.flagScoreLocation
    commander.issue(  commands.Charge, bot, target, description = 'Charge home (ATTACKER)')
    return True
...
def Command_RunToMiddlePerpen(commander, bot, pos):
    ...
    goalPos = middle+left*rnd*d.length()/2
...
def Command_AttackPathSmartStart(commander, bot, path):
    ...
    if len(path)>commander.level.firingDistance+2:
        d = int(commander.level.firingDistance)
        path = path[:d]
        points = commander.levelAnalysis.getAllVisiblePoints(path[d-1],commander.level.firingDistance*1.2, -1, 0, False, True)
        _, lookP = min([(commander.enemyDistMap[x][y] * (0.75+0.25*random.random()), (x,y)) for (x,y) in points if commander.enemyDistMap[x][y]>0 ], key=lambda pair:pair[0])
...
def Command_RunToEnemyFlagFlank4(commander, bot):
    fast=True
    pos = commander.freePos(commander.game.enemyTeam.flag.position)
    path = GetPathFromFlank(commander, bot,pos)
    if len(path)<=1:
...
def Command_AttackFlag(commander, bot):
    pos = commander.game.enemyTeam.flag.position
    commander.issue(commands.Attack, bot, pos, pos, description='Attack flag (ATTACKER)')
    return True


def Command_RunToRandomPos(commander, bot):
    pos = commander.level.findRandomFreePositionInBox(commander.level.area)
    commander.issue( commands.Charge, bot, pos, description = 'Charge to random point')
    return True

def Command_AttackRandomPos(commander, bot):
    pos = commander.level.findRandomFreePositionInBox(commander.level.area)
    commander.issue( commands.Attack, bot, pos, description = 'Attack random point')
    return True
...
def Condition_ReadyToAttack(commander, bot):
    result = (bot.state==BotInfo.STATE_SHOOTING or bot.state==BotInfo.STATE_TAKINGORDERS
               or 
               len([b for b in bot.visibleEnemies if b.health>0 and (b.position-bot.position).length()<commander.level.firingDistance+1 ])>0)
               #((bot.state==BotInfo.STATE_ATTACKING or bot.state==BotInfo.STATE_CHARGING)
               #   and len([b for b in bot.visibleEnemies 
               #            if b.health>0 and GetTimeToFightOptimistic(commander, bot, b)<Time_SwitchState ])>0))
    return result
...
def Command_ChargeEnemyFromFlank(commander, bot):
    path = GetPathFromFlank(commander, bot, bot.Enemy.position)
    if len(path)>0 and (len(path)<commander.level.runningSpeed*5 or bot.Enemy.state == BotInfo.STATE_DEFENDING):
        commander.issue(commands.Charge, bot, path, description='Charge enemy from flank');
        return True
    else:
        return False
...
def Command_AttackEnemyFromFlank(commander, bot):
    path = GetPathFromFlank(commander, bot, bot.Enemy.position)
    if len(path)>0 and (len(path)<commander.level.walkingSpeed*5 or bot.Enemy.state == BotInfo.STATE_DEFENDING):
...
def Command_MoveSafe(commander, bot):
    print bot.name, bot.path
    if len(bot.path)<=2:
        return False

    pos1 = bot.path[1]
    pos2 = bot.path[2]
    points = commander.levelAnalysis.getNewVisiblePoints(pos1,pos2)
    if len(points)<1:
        return False

    targetPosX, targetPosY = choice(points)
    commander.issue(commands.Attack, bot,bot.path[:4], Vector2(targetPosX+0.5, targetPosY+0.5),description='Move safe')
def Command_AttackTest(commander, bot):
    #print 'Attack direct'
    path = GetPathFromFlank(commander, bot, commander.game.enemyTeam.flag.position)
    commander.issue(commands.Attack, bot,path,description='Attack test')
...
def GetTimeToFight(commander, bot, enemy):
    ...
    stateDelta = 0
    if enemy.state == BotInfo.STATE_DEFENDING:
        if distance>commander.level.firingDistance:
            stateDelta = Time_SwitchState  
        else:
            return -1

    #if bot.state == BotInfo.STATE_DEFENDING:
    #    mySpeed = 0
    #elif bot.state == BotInfo.STATE_CHARGING or bot.state == BotInfo.STATE_MOVING: 
    #    mySpeed = commander.level.runningSpeed*1.0
    #elif bot.state == BotInfo.STATE_ATTACKING:
    #    mySpeed = commander.level.walkingSpeed*1.0
    speed = commander.level.runningSpeed*1.0
    if enemy.state == BotInfo.STATE_ATTACKING:
        speed = commander.level.walkingSpeed*1.0
    return (distance-commander.level.firingDistance)/speed+stateDelta


def GetTimeToFightOptimistic(commander, bot, enemy):
    if enemy in bot.seenBy:
        return GetTimeToFight(commander, bot, enemy)
    else:
        distance = (bot.position- enemy.position).length()
        if distance>commander.level.firingDistance:
            speed = commander.level.runningSpeed*1.0
            return (distance-commander.level.firingDistance)/speed + Time_SwitchState  
        else:
            return -1 
...
InterceptEnemy = BTSequence('Intercept My Flag',
    BTCondition(lambda commander,bot: commander.game.team.flag.carrier != None and (commander.game.enemyTeam.flagScoreLocation-bot.position).length()<(commander.game.enemyTeam.flagScoreLocation-commander.game.team.flag.position).length()+ commander.level.firingDistance,
        BTSelector(
            BTSequence(
                ...
                #SmartAttack
                    BTBotTask(lambda commander,bot: commander.issue(commands.Attack, bot, GetInterceptPoint(commander, bot,commander.game.team.flag.position, commander.game.enemyTeam.flagScoreLocation), description='Intercept Attack flag carrier (ATTACKER)'))
                )
            ),
            BTSequence(
...
ControlTerritory =  BTSequence('Control level',
    BTBotTask(lambda commander,bot:        
        Command_MoveToDefendPosition(
            commander, bot, commander.level.findRandomFreePositionInBox((commander.game.enemyTeam.flagSpawnLocation-commander.level.firingDistance*2,commander.game.enemyTeam.flagSpawnLocation+commander.level.firingDistance*2)))),
...
TakeFlag = BTSequence('TakeFlag',
    ...
    BTSelector(
        ...
        BTCondition(
            lambda commander,bot: commander.enemyBotsAlive<len(commander.game.bots_alive)/2 or (commander.unknownEnemiesCount==0 and commander.timeAfterRespawn>5),
            BTSequence(
        ...
        BTSequence(
            BTCondition(lambda commander,bot:  commander.EstimateTimeBeforeMeet(bot.position)>Time_SwitchState*2),
            BTCondition(lambda commander,bot:  commander.IsPosSafeNow(bot.position),
                BTSelector('Take flag',
                    BTSequence(
                        BTCondition(lambda commander,bot: (commander.game.enemyTeam.flag.position-bot.position).length()>commander.level.firingDistance*2),
                    ...
                    BTBotTask(lambda commander,bot: commander.issue(
                                    commands.Charge, bot, [commander.game.enemyTeam.flag.position,commander.game.team.flagScoreLocation], description='Move to flag (ATTACKER)'))
                )
            )
        ),

        BTSequence(
            BTCondition(lambda commander, bot: (commander.game.enemyTeam.flag.position-commander.game.team.flagScoreLocation).length()<commander.level.firingDistance
                        and (commander.game.enemyTeam.flag.position-bot.position).length()<commander.level.firingDistance),
            BTBotTask(lambda commander,bot: commander.issue(
                commands.Charge, bot, [commander.game.enemyTeam.flag.position,commander.game.team.flagScoreLocation], description='Run run to flag (ATTACKER)'))
               
        ),

        BTCondition((lambda commander,bot: commander.enemyBotsAlive>0 and ((commander.game.enemyTeam.flag.position-bot.position).length()>commander.level.firingDistance*1.2) or len(bot.visibleEnemies)>1),
            BTSelector(
                #BTSequence(
                ...
                BTLearningChoice('Learn Attack',
                    (30, BTBotTask(Command_AttackEnemyFlagFlankSmartStart)),
                    (30, BTBotTask(Command_AttackEnemyFlagFlankSmartStart)),
                    (30, BTBotTask(Command_RunToEnemyFlagFlank4)), 
                    (30, BTBotTask(Command_RunToEnemyFlagFlank2)),
                    (20, BTBotTask(Command_AttackFlag)),
                    (20, BTBotTask(Command_RunToEnemyFlag)),
                    (10, BTBotTask(Command_RunToRandomPos)),
                )
                #BTBotTask(lambda commander,bot:
                #    choice([Command_AttackEnemyFlagFlankSmart, 
                #            Command_AttackEnemyFlagFlankSmartStart, 
...
ReturnFlag = BTSequence(
    ...
    BTSelector(
        ...
        BTSequence(
            BTCondition(lambda commander,bot: commander.unknownEnemiesCountDelta<0),
            BTBotTask(Command_TryChargeHomeSafe)
        ),
        BTLearningChoice('LearnReturnFlag',
            (20, BTBotTask(Command_ChargeHomeSafe)),
            (10, BTBotTask(Command_RunHomeSafe)),
            (5, BTBotTask(Command_ChargeHome)),
            (2, BTBotTask(Command_RunHome))
        )
        #BTCondition(lambda commander,bot: commander.enemyBotsAlive>0,
...
SmartAttack = BTSequence('SmartAttack',
    BTCondition(lambda commander,bot: bot.Enemy!=None and bot.Enemy.health>0,
        ...
        #    ),
            ...
            BTSequence('Charge holding',
                BTCondition(lambda commander,bot: bot.Enemy.state == BotInfo.STATE_HOLDING),
                BTBotTask(Command_ChargeEnemyFromFlank),
            ),
            BTSequence('EnemySee',
                BTCondition(lambda commander,bot: len(bot.seenBy)>0),#bot in bot.Enemy.visibleEnemies),
                BTSelector(                    
                    BTSequence('EnemyAttacking',
                        BTCondition(lambda commander,bot: bot.Enemy.state == BotInfo.STATE_ATTACKING and GetTimeToFight(commander,bot,bot.Enemy)>Time_SwitchToDefend and GetTimeToFight(commander,bot,bot.Enemy)<Time_SwitchToDefend*2 ,
                            BTBotTask(lambda commander,bot:Command_DefendDirection(commander,bot,bot.Enemy.position-bot.position ))
                        )
                    ),
                    
                    BTCondition(lambda commander,bot: len(bot.visibleEnemies)<=1,
                        BTBotTask(Command_AttackEnemy))
                )
            ),
...
AttackerBTTree = BTTree(
    BTSelector(
        ...
        #BTBotTask(Command_MoveSafe),
        #BTBotTask(Command_AttackTest),
        ...
        BTSequence(
            ...
            BTCondition(lambda commander,bot: Condition_SetEnemyIfNotNone(commander,bot, commander.getBestEnemy(bot,commander.visibleEnemies)) 
                        ...
                        and (((bot.Enemy.position-bot.position).length()<commander.level.firingDistance*4
                                and(
                                (commander.game.enemyTeam.flag.position-bot.position).length()>(bot.Enemy.position-bot.position).length()
                            or (commander.game.team.flag.position-bot.position).length()>(bot.Enemy.position-commander.game.team.flag.position).length()
